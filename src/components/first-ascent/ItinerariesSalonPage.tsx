...
import { Crown, Gauge, Helicopter } from "lucide-react";
import Link from "next/link";

import { FIRST_ASCENT_LINKS } from "@/content/first-ascent";
import {
  ITINERARIES_AIRFRAMES,
  ITINERARIES_CABIN,
  ITINERARIES_DOSSIER,
  ITINERARIES_HERO,
  ITINERARIES_STRIP,
...
function openWhatsAppEnquiry(title: string, category = CATEGORY) {
  enquireWhatsApp(title, category);
}

export function ItinerariesSalonPage() {
  return (
    <div className="itineraries-salon first-ascent first-ascent-flush min-h-screen w-full bg-[#070709] text-neutral-100 selection:bg-[#C5A880]/30">
      <section className="relative mx-auto max-w-7xl px-6! pt-32! pb-24!">
        <nav className="mb-10! flex flex-wrap items-center justify-center gap-3" aria-label="First Ascent desks">
          {FIRST_ASCENT_LINKS.map((item) => {
            const active = item.href.endsWith("itineraries");
            return (
              <Link
                key={item.href}
                href={item.href}
        ...
        <div className="grid grid-cols-1 items-end gap-6 lg:grid-cols-12 lg:gap-5">
          ...
          >
            ...
            <div className="relative z-10">
              <p className="mb-4! font-mono text-[10px] tracking-[0.35em] text-[#C5A880] uppercase">
                {ITINERARIES_HERO.left.eyebrow}
              </p>
              <h1 className="mb-6! font-serif text-3xl leading-tight tracking-wider text-white uppercase md:text-4xl">
                {ITINERARIES_HERO.left.headline[0]}
                <br />
                {ITINERARIES_HERO.left.headline[1]}
              </h1>
              <button
                type="button"
                onClick={() => openWhatsAppEnquiry("Unrestricted Corridors")}
          ...
          >
            ...
            <h2 className="mb-6! font-serif text-3xl leading-tight tracking-wider text-white uppercase md:text-4xl">
              {ITINERARIES_HERO.right.headline[0]}
              <br />
              {ITINERARIES_HERO.right.headline[1]}
            </h2>
            <button
              type="button"
              onClick={() => openWhatsAppEnquiry("Diplomatic Clearance")}
              className="rounded-full border border-[#C5A880]/50 px-5! py-2! font-mono text-[10px] tracking-[0.28em] text-[#C5A880] uppercase transition-all duration-300 hover:bg-[#C5A880] hover:text-black"
            >
              {ITINERARIES_HERO.right.cta}
            </button>
          </motion.aside>
        </div>
      </section>
        ...
        {ITINERARIES_AIRFRAMES.map((airframe) => (
          ...
          >
            ...
            <div className="w-full">
              <p className="mb-2! font-mono text-[10px] tracking-[0.25em] text-neutral-400 uppercase">{airframe.eyebrow}</p>
              <h3 className="mb-2! font-serif text-2xl tracking-tight text-white">{airframe.title}</h3>
              <p className="mb-6! font-sans text-xs font-light leading-relaxed text-neutral-400">{airframe.spec}</p>
              <div className="flex w-full items-center justify-between border-t border-white/[0.08] pt-4!">
                <span className="font-mono text-[10px] tracking-widest text-neutral-400 uppercase">Standing Charter</span>
                <button
                  type="button"
                  onClick={() => openWhatsAppEnquiry(airframe.title)}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <article className="group relative h-[180px] overflow-hidden rounded-2xl border border-[#C5A880]/40">
            ...
            <span className="absolute top-4 left-4 rounded-full border border-[#C5A880]/70 bg-black/50 px-3! py-1! font-mono text-[10px] tracking-[0.22em] text-[#C5A880] uppercase">
              {ITINERARIES_CABIN.tag}
            </span>
            <button
              type="button"
              onClick={() => openWhatsAppEnquiry(ITINERARIES_CABIN.title)}
      ...
      <section className="mx-auto mb-28! grid max-w-7xl grid-cols-1 items-center gap-8 px-6! lg:grid-cols-12">
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-b from-[#18181D] to-[#0D0D10] lg:col-span-4">
          ...
          <p className="absolute right-4 bottom-4 z-10 font-mono text-[10px] tracking-[0.25em] text-[#C5A880] uppercase">
            {ITINERARIES_DOSSIER.tailfin.caption}
          </p>
        </div>

        <div className="flex flex-col gap-4 lg:col-span-4">
          {ITINERARIES_DOSSIER.specs.map((spec) => (
            <article
              key={spec.id}
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-b from-[#18181D] to-[#0D0D10] p-8! lg:col-span-4">
          ...
          />
          <div className="relative z-10">
            <p className="mb-2! font-mono text-xs tracking-[0.3em] text-[#C5A880] uppercase">Flight Desk Protocol</p>
            <p className="mb-6! font-sans text-xs font-light leading-relaxed text-neutral-400">
              {ITINERARIES_DOSSIER.manifesto}
            </p>

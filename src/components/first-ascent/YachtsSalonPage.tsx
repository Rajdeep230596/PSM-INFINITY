...
import { Gauge, Ship, Snowflake } from "lucide-react";
...
function openWhatsAppEnquiry(vesselName: string) {
  openWhatsApp(
    `Hello PSM Infinity, I would like to enquire about the Curated Maritime Charter allocation: ${vesselName}.`,
  );
}

export function YachtsSalonPage() {
  return (
    <div className="yachts-salon first-ascent first-ascent-flush min-h-screen w-full bg-[#070709] text-neutral-100 selection:bg-[#C5A880]/30">
      <section className="relative mx-auto max-w-7xl px-6! pt-32! pb-24!">
        <nav className="mb-10! flex flex-wrap items-center justify-center gap-3" aria-label="First Ascent desks">
          {FIRST_ASCENT_LINKS.map((item) => {
            const active = item.href.endsWith("yachts");
            return (
              <Link
                key={item.href}
                href={item.href}
        ...
        <div className="grid grid-cols-1 items-end gap-6 lg:grid-cols-12 lg:gap-5">
          ...
          >
            ...
            <div className="relative z-10">
              <p className="mb-4! font-mono text-[10px] tracking-[0.35em] text-[#C5A880] uppercase">
                {YACHTS_HERO.left.eyebrow}
              </p>
              <h1 className="mb-6! font-serif text-3xl leading-tight tracking-wider text-white uppercase md:text-4xl">
                {YACHTS_HERO.left.headline[0]}
                <br />
                {YACHTS_HERO.left.headline[1]}
              </h1>
              <button
                type="button"
                onClick={() => openWhatsAppEnquiry("Fleet Reach")}
          ...
          >
            ...
            />
            <img
              src={YACHTS_HERO.center.image}
              alt={YACHTS_HERO.center.imageAlt}
          ...
          >
            <p className="mb-4! font-mono text-[10px] tracking-[0.35em] text-[#C5A880] uppercase">
              {YACHTS_HERO.right.eyebrow}
            </p>
            <h2 className="mb-6! font-serif text-3xl leading-tight tracking-wider text-white uppercase md:text-4xl">
              {YACHTS_HERO.right.headline[0]}
              <br />
              {YACHTS_HERO.right.headline[1]}
            </h2>
            <button
              type="button"
              onClick={() => openWhatsAppEnquiry("Commission Charter")}
              className="rounded-full border border-[#C5A880]/50 px-5! py-2! font-mono text-[10px] tracking-[0.28em] text-[#C5A880] uppercase transition-all duration-300 hover:bg-[#C5A880] hover:text-black"
            >
              {YACHTS_HERO.right.cta}
            </button>
          </motion.aside>
        </div>
      </section>
        ...
        {YACHTS_VESSELS.map((vessel) => (
          ...
          >
            ...
            <div className="w-full">
              <p className="mb-2! font-mono text-[10px] tracking-[0.25em] text-neutral-400 uppercase">{vessel.eyebrow}</p>
              <h3 className="mb-2! font-serif text-2xl tracking-tight text-white">{vessel.title}</h3>
              <p className="mb-6! font-sans text-xs font-light leading-relaxed text-neutral-400">{vessel.spec}</p>
              <div className="flex w-full items-center justify-between border-t border-white/[0.08] pt-4!">
                <span className="font-mono text-[10px] tracking-widest text-neutral-400 uppercase">Private Allocation</span>
                <button
                  type="button"
                  onClick={() => openWhatsAppEnquiry(vessel.title)}
      ...
      <section className="mx-auto mb-28! max-w-7xl px-6!">
        <div className="mb-10! flex items-center justify-center gap-6">
          ...
          <h2 className="font-serif text-2xl tracking-tight text-white uppercase md:text-3xl">Maritime Classifications.</h2>
          <span className="hidden h-px flex-1 bg-white/[0.08] sm:block" />
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {YACHTS_TIERS.map((tier) => {
          ...
          <article className="group relative h-[180px] overflow-hidden rounded-2xl border border-[#C5A880]/40">
            <img
              src={YACHTS_DECK.image}
              alt={YACHTS_DECK.imageAlt}
            ...
            <button
              type="button"
              onClick={() => openWhatsAppEnquiry(YACHTS_DECK.title)}
      ...
      <section className="mx-auto mb-28! grid max-w-7xl grid-cols-1 items-center gap-8 px-6! lg:grid-cols-12">
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-b from-[#18181D] to-[#0D0D10] lg:col-span-4">
          ...
          />
          <img
            src={YACHTS_DOSSIER.bow.image}
            alt={YACHTS_DOSSIER.bow.imageAlt}
          ...
          <p className="absolute right-4 bottom-4 z-10 font-mono text-[10px] tracking-[0.25em] text-[#C5A880] uppercase">
            {YACHTS_DOSSIER.bow.caption}
          </p>
        </div>

        <div className="flex flex-col gap-4 lg:col-span-4">
          {YACHTS_DOSSIER.specs.map((spec) => (
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-b from-[#18181D] to-[#0D0D10] p-8! lg:col-span-4">
          ...
          />
          <div className="relative z-10">
            <p className="mb-2! font-mono text-xs tracking-[0.3em] text-[#C5A880] uppercase">Maritime Desk Protocol</p>
            <p className="mb-6! font-sans text-xs font-light leading-relaxed text-neutral-400">{YACHTS_DOSSIER.manifesto}</p>
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              <button
                type="button"
                onClick={() => openWhatsAppEnquiry("Manifest Request")}
              ...
              <button
                type="button"
                onClick={() => openWhatsAppEnquiry("Berth Dossier")}

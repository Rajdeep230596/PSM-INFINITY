...
import {
  ...
  COLLECTIBLES_COMMISSIONS,
  COLLECTIBLES_HERO,
  COLLECTIBLES_JOURNAL,
  COLLECTIBLES_PILLARS,
  COLLECTIBLES_SPOTLIGHTS,
  COLLECTIBLES_TRUST,
...
export function CollectiblesSalonPage() {
  ...
  const openConcierge = (itemId?: string) => {
    const commission = COLLECTIBLES_COMMISSIONS.find((item) => item.id === itemId || item.title === itemId);
    const spotlight = COLLECTIBLES_SPOTLIGHTS.find((item) => item.id === itemId);
    const journal = COLLECTIBLES_JOURNAL.find((item) => item.id === itemId);
    const nextBrief = commission
      ? `Request allocation: ${commission.title}`
      : spotlight
        ...
        : journal
          ...
          : itemId
            ? itemId
            : "Private folio — collectibles desk.";
    setSent(false);
    setBrief(nextBrief);
    setDeskOpen(true);
  };
  ...
  const submitDispatch = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!mandate) return;
    setDispatchNote("Mandate received. A desk will open protocol.");
    openConcierge(`Acquisition brief: ${mandate}`);
    event.currentTarget.reset();
  };

  return (
    <div className="collectibles-salon first-ascent first-ascent-flush min-h-screen w-full bg-[#08080A] text-neutral-100">
      <section className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 px-6! pt-36! pb-20! sm:px-8! lg:grid-cols-12">
        <div className="lg:col-span-5">
          <h1 className="mb-4! font-serif text-4xl font-light leading-tight tracking-tight text-white md:text-6xl">
            {COLLECTIBLES_HERO.heading}
          </h1>
          <p className="mb-8! max-w-md font-sans text-sm font-light text-neutral-400">{COLLECTIBLES_HERO.subhead}</p>
          <button
            type="button"
            onClick={() => openConcierge("Request acquisition folio")}
        ...
        <div className="relative lg:col-span-7">
          ...
          <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-[#121215] shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
            <img
              src="/assets/collectibles/hero-watch-hypercar.jpg"
              alt="Rose-gold tourbillon on carbon bodywork with a hypercar in the bay"
      ...
      <nav
        id="categories"
        aria-label="Collectibles categories"
      ...
      >
        <ul className="flex min-w-max items-start justify-center gap-6 py-2! sm:gap-8 lg:gap-10">
          {COLLECTIBLES_CATEGORIES.map((category) => {
      ...
      <section className="mx-auto mb-24! grid max-w-7xl grid-cols-1 gap-6 px-6! md:grid-cols-3">
        {COLLECTIBLES_SPOTLIGHTS.map((card) => (
          <article
            key={card.id}
            id={card.id}
      ...
      <section id="allocations" className="scroll-mt-28">
        <div className="mx-auto mb-8! flex max-w-7xl items-end justify-between px-6!">
          ...
          <button
            type="button"
            onClick={() => openConcierge("Request the complete acquisition vault")}
        ...
        <div className="mx-auto mb-28! grid max-w-7xl grid-cols-1 gap-6 px-6! sm:grid-cols-2 lg:grid-cols-4">
          {COLLECTIBLES_COMMISSIONS.map((item) => (
            <article
              key={item.id}
              id={item.category}
      ...
      <section className="mx-auto mb-28! max-w-7xl px-6!">
        <div className="relative flex min-h-[300px] items-center justify-between overflow-hidden rounded-3xl border border-white/[0.08] p-10! md:p-16!">
          <img
            src="/assets/collectibles/editorial-vault.jpg"
            alt="Underground collector vault with hypercars and vitrines of timepieces"
          ...
          <div className="relative z-10 max-w-xl">
            <p className="mb-3! font-mono text-[10px] tracking-[0.25em] text-[#C5A880] uppercase md:text-[11px]">
              Private Vault · Strict Chain of Custody
            </p>
            <h2 className="mb-4! font-serif text-3xl font-light tracking-tight text-white md:text-4xl">
              Rare Mechanical Art Circulated in Complete Discretion.
            </h2>
            <button
              type="button"
              onClick={() => openConcierge("Request vault access")}
      ...
      <section id="journal" className="scroll-mt-28">
        ...
        <div className="mx-auto mb-24! grid max-w-7xl grid-cols-1 gap-6 px-6! md:grid-cols-3">
          {COLLECTIBLES_JOURNAL.map((entry) => (
            <article
              key={entry.id}
      ...
      <section className="mx-auto mb-16! grid max-w-7xl grid-cols-1 gap-8 px-6! sm:grid-cols-2 lg:grid-cols-4">
        {COLLECTIBLES_PILLARS.map((pillar) => {
        ...
        >
          <div className="w-full sm:max-w-md">
            ...
            <p className="font-sans text-xs font-light leading-relaxed text-neutral-400 md:text-sm">
              Name the reference, chassis, or acquisition brief. A private desk will open protocol.
            </p>
            {dispatchNote ? <p className="mt-2! text-xs text-[#C5A880]">{dispatchNote}</p> : null}
          </div>
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
            <label className="sr-only" htmlFor="collectibles-mandate">
              Reference number, chassis model, or acquisition brief
            </label>
            <input
              id="collectibles-mandate"
              name="mandate"
              type="text"
              required
              placeholder="Enter reference number, chassis model, or acquisition brief"
            ...
            >
              ...
              {sent ? (
                <div className="flex flex-1 flex-col items-center justify-center text-center">
                  ...
                  <p className="font-serif text-2xl font-light text-[#F5F2EC]">Mandate received</p>
                ...
                <form onSubmit={submit} className="flex flex-1 flex-col gap-8">
                  ...
                  <label className="block">
                    ...
                    <textarea
                      ...
                      placeholder="Any reference, chassis, or off-market mandate — we stage the viewing."

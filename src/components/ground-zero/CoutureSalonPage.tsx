...
export function CoutureSalonPage() {
  ...
  return (
    <div className="couture-salon first-ascent first-ascent-flush min-h-screen w-full bg-[#08080A] text-neutral-100">
      <section className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 px-6! pt-36! pb-20! sm:px-8! lg:grid-cols-12">
        <div className="lg:col-span-5">
          <h1 className="mb-4! font-serif text-4xl font-light leading-tight tracking-tight text-white md:text-6xl">
            {COUTURE_HERO.heading}
          </h1>
          <p className="mb-8! max-w-md text-sm font-light text-neutral-400">{COUTURE_HERO.subhead}</p>
          <button
            type="button"
      ...
      >
        <ul className="flex min-w-max items-start justify-center gap-6 py-2! sm:gap-8 lg:gap-10">
          {COUTURE_CATEGORIES.map((category) => {
            const Icon = CATEGORY_ICONS[category.icon];
            return (
              <li key={category.id} className="flex w-[4.75rem] flex-col items-center text-center sm:w-20">
                <a
                  href={`#${category.id}`}
        ...
        {COUTURE_SPOTLIGHTS.map((card) => (
          ...
          >
            <div className="relative z-10 max-w-[190px]">
              ...
              <h2 className="mb-2! font-serif text-xl text-white">{card.title}</h2>
      ...
      <section id="allocations" className="scroll-mt-28">
        <div className="mx-auto mb-8! flex max-w-7xl items-end justify-between px-6!">
          <div>
            ...
            <h2 className="font-serif text-2xl font-light text-[#F5F2EC] md:text-3xl">Private wardrobe commissions</h2>
          </div>
          <button
            type="button"
            onClick={() => handleEnquiry("Request the complete allocation lookbook")}
          ...
          {COUTURE_ALLOCATIONS.map((item) => (
            ...
            >
              <div>
                ...
                <p className="mb-4! text-xs font-light text-neutral-400">{item.spec}</p>
              </div>
        ...
        <div className="relative flex min-h-[280px] items-center justify-between overflow-hidden rounded-3xl border border-white/[0.08] p-10! md:p-16!">
          ...
          <div className="relative z-10 max-w-xl">
            <p className="mb-3! font-mono text-[10px] tracking-[0.25em] text-[#C5A880] uppercase md:text-[11px]">
              Atelier Archives · Private Commission
            </p>
            <h2 className="mb-4! font-serif text-3xl text-white md:text-4xl">Handcrafted Elegance Without Compromise</h2>
            <button
              type="button"
      ...
      <section id="journal" className="scroll-mt-28">
        <div className="mx-auto mb-8! flex max-w-7xl items-end justify-between px-6!">
          <div>
            ...
            <h2 className="font-serif text-2xl font-light text-[#F5F2EC] md:text-3xl">Curator&apos;s Journal</h2>
          </div>
          <button
            type="button"
            onClick={() => handleEnquiry("Request the complete curator journal")}
            className="border-0 bg-transparent appearance-none font-mono text-[10px] tracking-[0.22em] text-neutral-400 uppercase transition-colors hover:text-[#C5A880] md:text-[11px]"
          >
            See All ↗
          </button>
        ...
        {COUTURE_PILLARS.map((pillar) => {
          ...
          return (
            <article key={pillar.id} className="flex items-start gap-3">
              ...
              <div>
                <h3 className="mb-1! font-serif text-sm text-[#F5F2EC]">{pillar.title}</h3>
                <p className="text-xs leading-relaxed font-light text-neutral-400 md:text-sm">{pillar.body}</p>
              </div>
            </article>
          );
        })}
      ...
      <section id="dispatch" className="mx-auto mb-20! max-w-7xl px-6!">
        <form
          onSubmit={submitDispatch}
          className="flex flex-col items-center justify-between gap-4 rounded-2xl border border-white/10 bg-[#121215] p-6! sm:flex-row"
        >
          <div className="w-full sm:max-w-md">
            <p className="mb-1! font-mono text-[10px] tracking-[0.25em] text-[#C5A880] uppercase md:text-[11px]">
              Private Dispatch
            </p>
            <p className="text-xs leading-relaxed font-light text-neutral-400 md:text-sm">
              Be the first to know about new allocations, exclusive viewings, and atelier notes.
            </p>
            {dispatchNote ? <p className="mt-2! text-xs text-[#C5A880]">{dispatchNote}</p> : null}
          </div>
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
            <label className="sr-only" htmlFor="couture-dispatch-email">
              Email address
            </label>
            <input
              id="couture-dispatch-email"
              name="email"
              type="email"
              required
              placeholder="Enter your email address"
              className="w-full rounded-full border border-white/10 bg-[#08080A] px-5! py-3! text-sm font-light text-[#F5F2EC] outline-none placeholder:text-neutral-500 focus:border-[#C5A880]/50 sm:w-72"
            />
            <button
              type="submit"
